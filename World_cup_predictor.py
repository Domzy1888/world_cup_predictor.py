...
# Global Transparent Glassmorphism Overlay Injection
st.markdown("""
    <style>
    ...
    /* Global Glassmorphism Wrapper for Native Elements */
    .glass-panel {
    ...
    /* Target Streamlit's internal column containers to force glass visual framing */
    div[data-testid="stVerticalBlock"] > div:has(div.match-card-anchor) {
    ...
    /* Ensure clean typography contrast over mobile/desktop */
    .stMain, h1, h2, h3, h4, p, label {
    ...
    /* Ensure selectboxes match dark theme UI guidelines */
...
with col_nav1:
    st.markdown(f"👥 Account: **{c_user}** " + ("<span style='color:#60a5fa;'>(🛠️ Admin)</span>" if st.session_state.users[c_user]["is_admin"] else ""), unsafe_allow_html=True)
with col_nav2:
    ...
    with pred_sub_tabs[0]:
        ...
        with col_input:
            ...
            for idx, (home, away) in enumerate(MATCH_IDS[selected_group]):
                ...
                # Structural anchor pairing widget inside the custom glass container box
                with st.container():
                    st.markdown('<div class="match-card-anchor"></div>', unsafe_allow_html=True)
                    ...
                    st.markdown(f"<div class='team-header-text'>{fmt_team(away)}</div>", unsafe_allow_html=True)
                    user_preds[ka] = st.number_input("Away Score", min_value=0, max_value=15, value=user_preds.get(ka, 0), step=1, key=f"p_{ka}", disabled=is_group_locked)
        ...
        with ko_tabs[0]:
            for m_id, (h, a) in o_r32.items():
                with st.container():
                    ...
                    user_preds[m_id] = st.selectbox("Progresses:", options, index=options.index(curr) if curr in options else 0, format_func=fmt_team, key=f"up_sel_{m_id}")
